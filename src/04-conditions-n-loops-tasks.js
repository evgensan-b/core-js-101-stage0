...
function isInsideCircle(circle, point) {
  ...
  const distSquared = difX * difX + difY * difY;
...
function isCreditCardNumber(ccn) {
  ...
  const digits = ccnString.split('').map(Number);
...
function getDigitalRoot(num) {
  ...
  while (result > 9) {
    ...
    result = arr.reduce((sum, currNum) => sum + Number(currNum), 0);
  }
...
function getCommonDirectoryPath(pathes) {
  const commonParts = pathes[0].split('/');
  commonParts.pop();
